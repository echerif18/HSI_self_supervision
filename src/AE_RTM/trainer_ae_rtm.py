...
#         self.log_epoch = 10
#         self.scaler_model = None
#         self.loss_recons_criterion = CosineSimilarityLoss() #mse_loss
#         self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#         self.lamb = 1e0
...
#         self.model = AE_RTM_corr(self.settings.input_shape, self.settings.n_lb, scaler_list=self.settings.scaler_model)
#         self.transformation_setup()
#         self.criterion_setup()

    
#     def prepare_optimizers(self, num_epochs):
#         """Prepares optimizers with learning rate warmup and decay."""
...
#     def criterion_setup(self):
#         self.criterion = self.settings.loss_recons_criterion #CosineSimilarityLoss() #mse_loss
...
#     def grad_stabilizer(self):
#         epsilon = 1e-7  # Minimum stabilizing value
#         min_threshold = 1e-5  # Prevent vanishing gradients
        
#         # Identify gradients that are NaN or too small
#         para_grads = [v.grad for v in self.model.parameters()
#                       if v.grad is not None and (torch.isnan(v.grad).any() or torch.abs(v.grad).max() < min_threshold)]
        
#         if len(para_grads) > 0:
#             for v in para_grads:
#                 rand_values = torch.rand_like(v, dtype=torch.float) * epsilon  # Small random values
#                 mask_nan = torch.isnan(v)
#                 mask_small = torch.abs(v) < min_threshold
    
#                 # Replace NaNs and small gradients safely
#                 v.copy_(torch.where(mask_nan, rand_values, v))  # Replace NaNs
#                 v.copy_(torch.where(mask_small, v + epsilon, v))  # Boost very small gradients
    
#             self.stablize_count += 1  # Track stabilization steps
#             # print(f"🛠 Gradient Stabilization Applied | Count: {self.stablize_count}")
...
#         if (self.settings.type is not 'full'):
#             output = out[:,:self.settings.input_shape] ### half-range ##
...
#         sup_idx = torch.tensor([0,1,2,3,4,6,7]).to(self.settings.device) ## eliminate indices
            
#         labeled_loss_function = HuberCustomLoss(threshold=1.0)
#         loss_lb = labeled_loss_function(torch.index_select(x, 1, sup_idx), torch.index_select(lb_bx, 1, sup_idx))

#         recon_weight = loss_recos.item() / (loss_lb.item() + 1e-6)  # Prevent division by zero
#         loss = (recon_weight * loss_recos) + loss_lb
...
#                 if grad_norm < 1e-7:
#                     print(f"⚠️ Warning: Vanishing gradient detected in {name} | Norm: {grad_norm:.10f}")
#                     param.grad.add_(torch.randn_like(param.grad) * 1e-6)  # Boost gradients artificially
...
#                 ## half-range!!!
#                 if (self.settings.type is not 'full'):
#                     data_val = data_val.view(data_val.shape[0], data_val.shape[-1])[:,:self.settings.input_shape].to(self.settings.device) ####half_range 
                
#                 lb_bx_val = lb_bx_val.to(self.settings.device).float()
#                 data_val = data_val.squeeze().float().to(self.settings.device)
#                 # data_val = data_val.unsqueeze(dim=1).float()[:,:,:].to(self.settings.device) #-1 #cnn
                  
                
#                 if(self.settings.scaler_model is not None):
#                     # scaling_layer = scaler_layer(model.scaler_list) #model.scaler_layer() #scaler_list
#                     # transformation_layer = StaticTransformationLayer(transformation=scaling_layer).to(self.settings.device)
#                     lb_bx_val = self.transformation_layer(lb_bx_val)
...
#                 if (self.settings.type is not 'full'):
#                     output_val = output_val[:,:self.settings.input_shape] ### half-range ##
    
#                 data_val = data_val[~torch.any(output_val.isnan(),dim=1)] 
#                 # data_val = data_val.squeeze()[~torch.any(output_val.isnan(),dim=1)] #cnn
#                 output_val = output_val[~torch.any(output_val.isnan(),dim=1)]
...
#                 # target = torch.ones(data_val.size(0)).to(self.settings.device)
#                 # val_loss_recos = criterion(output_val, data_val, target)
                
#                 sup_idx = torch.tensor([0,1,2,3,4,6,7]).to(self.settings.device) ## eliminate indices
#                 # sup_idx = torch.tensor([0,1,3,4,5,6,7]).to(self.settings.device) ## eliminate indices
#                 # val_loss_lb = labeled_loss_function(torch.index_select(x_val, 1, sup_idx), torch.index_select(lb_bx_val, 1, sup_idx), threshold=1)
                
#                 labeled_loss_function = HuberCustomLoss(threshold=1.0)
#                 val_loss_lb = labeled_loss_function(torch.index_select(x_val, 1, sup_idx), torch.index_select(lb_bx_val, 1, sup_idx))
#                 loss_lb_val += val_loss_lb
                
#                 r2_val += r_squared(lb_bx_val.detach(), x_val.detach())
#                 # r2_val += r_squared(lb_bx_val.detach(), x_val.detach(), scaler=self.settings.scaler_model)
                
#                 # val_loss_lb = labeled_loss_function(x_val, lb_bx_val, threshold=1)
                
#                 # lamb = 1e2
#                 recon_weight = val_loss_recos.item() / (val_loss_lb.item() + 1e-6)  # Prevent division by zero
#                 val_loss = (recon_weight * val_loss_recos) + val_loss_lb
#                 # val_loss =  self.settings.lamb * (val_loss_recos +  mae_loss_brightness(output_val, data_val)) +  val_loss_lb +  mae_loss_brightness(output_val, data_val) #1.5 *self.settings.lamb *
#                 tot_val += val_loss
...
#         if self.settings.scaler_model is not None:
#             ### with sacler ####]
#             ori_lb = pd.DataFrame(self.settings.scaler_model.inverse_transform(ori.cpu().detach().numpy()), columns=ls_tr[:])#[original_dict.keys()]
#             # display(ori_lb.describe())
            
#             df_tr_val = pd.DataFrame(self.settings.scaler_model.inverse_transform(preds.cpu().detach().numpy()), columns=ls_tr)
#             df_tr_val['cbc'] = df_tr_val['cm']-df_tr_val['cp']
...
#             for batch_idx, samples in enumerate(tqdm(infinite_iter(self.train_loader, self.unlabeled_loader), total=len(self.unlabeled_loader), desc=f'Training epoch {epoch}')):
#                 if (self.settings.type is not 'full'):
#                     sp , lb = samples
#                     sp = sp.view(sp.shape[0], sp.shape[-1])[:,:self.settings.input_shape].to(self.settings.device) ####half_range
#                     samples = (sp, lb)
#                 loss_recos, loss_lb, loss, r2_train_step = self.train_step(samples)
...
#                     metrics_per_step.update({'train_step/step': batch_idx+1})
#                     metrics_per_step.update({'train_step/train_reconsloss': loss_recos})
#                     metrics_per_step.update({'train_step/train_lbloss': loss_lb})
#                     metrics_per_step.update({'train_step/train_r2': r2_train_step})
#                     wandb.log(metrics_per_step) #, step=batch_idx+1
            
#             loss_epoch = loss_epoch/(len(self.unlabeled_loader))    ## repeated
#             loss_lb_epoch = loss_lb_epoch/(len(self.unlabeled_loader))    ## repeated
#             r2_epoch = r2_epoch/(len(self.unlabeled_loader))    
...
#                 # metrics_per_epoch = {'train_epoch/train_r2': r2_epoch}
#                 metrics_per_epoch.update({'train_epoch/train_r2': r2_epoch})
#                 metrics_per_epoch.update({'train_epoch/lr': self.optimizer.param_groups[0]['lr']}) #optimizer.param_groups[0]['lr'] scheduler.get_last_lr()[0] 'learning_rate_D': self.d_optimizer.param_groups[0]['lr']
#                 wandb.log(metrics_per_epoch)
        
#             # if(epoch%self.settings.log_epoch==0):
#             # val_loss_epoch, r2_val = self.val_epoch()
#             loss_recons_val, loss_lb_val, val_loss_epoch, r2_val = self.val_epoch()
            
#             if(self.settings.logger is not None):
#                 val_metrics_per_epoch = {'train_epoch/val_loss': val_loss_epoch}
#                 val_metrics_per_epoch.update({'train_epoch/val_reconsloss': loss_recons_val})
#                 val_metrics_per_epoch.update({'train_epoch/val_lbloss': loss_lb_val})
#                 val_metrics_per_epoch.update({'train_epoch/val_r2': r2_val}) #r2_score
#                 wandb.log(val_metrics_per_epoch) #, step=batch_idx+1
...
#                         .format(epoch, loss_epoch, val_loss_epoch, r2_epoch, r2_val))

#             #### this saves the best model but does not stop the training!!
#             if(self.early_stopping is not None):
