...
def grad_stabilizer(model, epoch, batch_idx, loss, stablize_count):
    epsilon = 1e-7  # Minimum stabilizing value
    min_threshold = 1e-5  # Prevent vanishing gradients
    
    para_grads = [v.grad for v in model.parameters()
    ...
    if len(para_grads) > 0:
        for v in para_grads:
            ...
            # Replace NaNs and small gradients
            v = torch.where(mask_nan, rand_values, v)
            v = torch.where(mask_small, v + epsilon, v)

        stablize_count += 1
        # print(f"🛠 Gradient Stabilization Applied at Epoch {epoch}, Batch {batch_idx}")

    return stablize_count, para_grads
