        ...
        for e in range(epoch_start, num_epochs + 1):
            ...
            for images in tqdm(
                ...
                if(self.settings.type != 'full'):
                    # images = images.view(images.shape[0], images.shape[-1])[:,:self.settings.n_bands].to(self.settings.device) ####half_range
            ...
            # # Step the appropriate learning rate scheduler based on the current epoch
            # if e < self.warmup_epochs:
            #     self.warmup_scheduler.step()  # Warmup phase
            # else:
            #     if self.scheduler_mode == "warmup":
            #         print(f"🔥 Switching from Warmup to Cosine Annealing at epoch {e}")
            #         self.scheduler_mode = "cosine"
            #     self.decay_scheduler.step()  # Cosine annealing phase
