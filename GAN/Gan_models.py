...
class Generator(nn.Module):
    def __init__(self, latent_dim, input_shape):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.ConvTranspose1d(64, 51, kernel_size=4, stride=2, padding=1), # Upsample to 850
            nn.LeakyReLU(0.2),
            nn.BatchNorm1d(51),
            
            nn.ConvTranspose1d(51, 51, kernel_size=4, stride=2, padding=1), # Upsample to 1700
            nn.LeakyReLU(0.2),
            nn.BatchNorm1d(51),
            
            nn.Conv1d(51, 1, kernel_size=7, padding=3), # Output
            nn.Tanh()
        )

    def forward(self, x):
        x = self.fc(x)
        x = x.view(-1, 64, round(self.input_shape/4)) # Reshape to fit Conv1D input shape
